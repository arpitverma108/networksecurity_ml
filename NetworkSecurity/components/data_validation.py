from NetworkSecurity.entity.aftifact_entity import DataIngestionArtifact, DataValidationArtifact
...
class DataValidation:
    ...
    def validate_numerical_columns(self, dataframe: pd.DataFrame) -> bool:
        """
        Validates that all required numerical columns exist in the dataframe
        and have numeric datatypes (int64 or float64).
        """
        try:
            ...
            missing_columns = [col for col in expected_numerical_columns if col not in actual_columns]
            ...
            if non_numeric_columns:
                logging.error(f"❌ These columns are not numeric: {non_numeric_columns}")
                return False

            logging.info("✅ All numerical columns exist and have correct data types.")
            return True
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                raise Exception("Train dataframe failed numerical column validation.")

            status = self.validate_numerical_columns(test_dataframe)
            if not status:
                raise Exception("Test dataframe failed numerical column validation.")
